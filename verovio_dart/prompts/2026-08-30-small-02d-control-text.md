# 02d — view_control.dart: drawControlElementText, drawTextEnclosure

## Contexto (não precisa ler mais nada)

Você vai tipar 2 método(s) de `lib/src/rendering/view_control.dart`, tirando `as dynamic` e `catch (_)`. Execução mecânica via helper `_dyn` (ver relatório medium-02).
...
|---|---|---|---|---|
| `drawControlElementText` | — | — | — | `origin/src/src/view_control.cpp` |
| `drawTextEnclosure` | — | — | — | `origin/src/src/view_control.cpp` |

**Total pontos desta unidade:** ~42
**Famílias afetadas:** dir, tempo, cpmark
...
```bash
tool/task_check.sh view_control.dart dir tempo cpmark
```
