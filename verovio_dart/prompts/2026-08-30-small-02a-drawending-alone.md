# 02a — view_control.dart: drawEnding

## Contexto (não precisa ler mais nada)

Você vai tipar 1 método(s) de `lib/src/rendering/view_control.dart`, tirando `as dynamic` e `catch (_)`. Execução mecânica via helper `_dyn` (ver relatório medium-02).
...
|---|---|---|---|---|
| `drawEnding` | — | — | — | `origin/src/src/view_control.cpp` |

**Total pontos desta unidade:** ~73
**Famílias afetadas:** ending
...
```bash
tool/task_check.sh view_control.dart ending
```
