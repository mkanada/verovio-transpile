...
### Métrica Geral (primário: estrutural; fallback: numérico)
...
**Baseline é dupla, mas o objetivo do loop é um por vez:**
...
- O loop mira **estrutural enquanto houver divergências estruturais** (X < 621). Quando X = 621 (passa a mirar **numérico**, e usa Y).
- A escolha do "alvo ativo" é sua: se estrutural > 0, alvo é estrutural; senão, alvo é numérico.
...
### Fluxo
1. **INÍCIO — Baseline Geral:** Determine `alvo_ativo` (= estrutural se
   `erros_est_antes > 0` na linha 3, senão numérico pela linha 4). Leia
   `erros_antes` do alvo ativo aplicando o reaproveitamento seguro
   descrito em "Métrica Geral" (reaproveita se as 3 condições valerem;
   senão roda `compare_svg --all`, 700s, do zero). Rode `dart
   analyze`/`dart test` normalmente (são rápidos — segundos e poucos
   minutos — e não têm um artefato persistido equivalente ao
   SVG_VALIDATION.md para reaproveitar com a mesma segurança).
...
3. **Tentativa 1..4 (por teste):**
   ...
   c. **Só depois:** Rode verificação **geral** `compare_svg --all` (700s) → `erros_depois` (do alvo ativo) + `dart analyze` + `dart test`.
   d. **Decisão sua:**
      - Se `erros_depois < erros_antes` **E** analyze/test não pioraram: `git add -A && git commit -m "fix: svg <alvo> <antes>-><depois> [loop auto] <arq>" && git push origin main` → SUCESSO, encerre. (alvo ∈ {est, num}.)
...
Reporte: baseline geral (est, num, alvo ativo), teste focado escolhido, cada tentativa (causa, correção, single-test alvo antes/depois, geral antes/depois, regressões identificadas), decisão final.
