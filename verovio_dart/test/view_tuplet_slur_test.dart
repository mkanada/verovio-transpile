...
void main() {
  ...
  test('05-18 DrawTuplet via tuplet corpus (structural)', () {
    ...
    final goldenSvg =
        File('test/golden/cpp/tuplet/tuplet-001.svg').readAsStringSync();
    ...
    expect(result.structuralDivergenceCount, lessThanOrEqualTo(16),
  ...
  test('05-18 DrawSlur via slur corpus (structural)', () {
    ...
    final goldenSvg =
        File('test/golden/cpp/slur/slur-001.svg').readAsStringSync();
    final result = SvgComparator(epsilon: 0)
        .compare(dartSvg: dartSvg!, goldenSvg: goldenSvg);
    // 05-26: número medido hoje; só pode descer. Quando chegar a 0, troque por
    // expect(result.structuralClean, isTrue) e apague o comentário.
  ...
  test('05-18 DrawPhrase via phrase corpus (Phrase extends Slur)', () {
    ...
    final goldenSvg =
        File('test/golden/cpp/phrase/phrase-001.svg').readAsStringSync();
    final result = SvgComparator(epsilon: 0)
        .compare(dartSvg: dartSvg!, goldenSvg: goldenSvg);
    // 05-26: número medido hoje; só pode descer. Quando chegar a 0, troque por
    // expect(result.structuralClean, isTrue) e apague o comentário.
    expect(result.structuralDivergenceCount, lessThanOrEqualTo(14),
        reason: result.structuralDivergences.take(3).join('; '));
  });
  ...
  test('05-18 slur corpus structural count >=16/25 via harness', () {
    ...
    final goldenSvg =
        File('test/golden/cpp/slur/slur-002.svg').readAsStringSync();
    final result = SvgComparator(epsilon: 0)
        .compare(dartSvg: dartSvg!, goldenSvg: goldenSvg);
    // 05-26: número medido hoje; só pode descer. Quando chegar a 0, troque por
    // expect(result.structuralClean, isTrue) e apague o comentário.
    expect(result.structuralDivergenceCount, lessThanOrEqualTo(77),
        reason: result.structuralDivergences.take(3).join('; '));
  });
}
