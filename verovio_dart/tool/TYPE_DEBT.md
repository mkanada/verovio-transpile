...
Lavagem de tipo (_dyn + as dynamic + declarações dynamic): 0
Engolidores silenciosos (catch sem rethrow nem log): 0
Supressões de erro de tipo: 0
Dívida total (D = A + B + C): 0

Gerado em 2026-09-06 por `dart run tool/debt_report.dart`.

- A.1 chamadas `_dyn(...)` (exclui as 3 linhas de declaração do helper): 0
- A.2 `as dynamic`: 0
- A.3 declarações/parâmetros `dynamic x` (exclui o helper): 0
...
| view_mensural.dart | 910 | 0 (0/0/0) | 0 / 0 | 0 | 0 |
...
| view_text.dart | 971 | 0 (0/0/0) | 0 / 0 | 0 | 0 |
