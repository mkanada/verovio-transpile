...
Lavagem de tipo (_dyn + as dynamic + declarações dynamic): 226
Engolidores silenciosos (catch sem rethrow nem log): 3
Supressões de erro de tipo: 0
Dívida total (D = A + B + C): 229

Gerado em 2026-09-05 por `dart run tool/debt_report.dart`.

- A.1 chamadas `_dyn(...)` (exclui as 3 linhas de declaração do helper): 156
- A.2 `as dynamic`: 0
- A.3 declarações/parâmetros `dynamic x` (exclui o helper): 70
...
| view_control.dart | 3832 | 102 (73/0/29) | 1 / 1 | 0 | 103 |
