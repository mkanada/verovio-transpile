...
Lavagem de tipo (_dyn + as dynamic + declarações dynamic): 385
Engolidores silenciosos (catch sem rethrow nem log): 34
Supressões de erro de tipo: 0
Dívida total (D = A + B + C): 419

Gerado em 2026-09-05 por `dart run tool/debt_report.dart`.

- A.1 chamadas `_dyn(...)` (exclui as 3 linhas de declaração do helper): 274
- A.2 `as dynamic`: 0
- A.3 declarações/parâmetros `dynamic x` (exclui o helper): 111
- B.1 total de `catch` no diretório: 34
- B.2 dos quais sem `rethrow` nem log (contam para B): 34
...
| view_mensural.dart | 959 | 33 (25/0/8) | 3 / 3 | 0 | 36 |
