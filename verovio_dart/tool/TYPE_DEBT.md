...
Lavagem de tipo (_dyn + as dynamic + declarações dynamic): 135
Engolidores silenciosos (catch sem rethrow nem log): 2
Supressões de erro de tipo: 0
Dívida total (D = A + B + C): 137

Gerado em 2026-09-05 por `dart run tool/debt_report.dart`.

- A.1 chamadas `_dyn(...)` (exclui as 3 linhas de declaração do helper): 92
- A.2 `as dynamic`: 0
- A.3 declarações/parâmetros `dynamic x` (exclui o helper): 43
...
| view_element.dart | 3327 | 83 (54/0/29) | 2 / 2 | 0 | 85 |
