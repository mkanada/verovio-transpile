...
Lavagem de tipo (_dyn + as dynamic + declarações dynamic): 149
Engolidores silenciosos (catch sem rethrow nem log): 2
Supressões de erro de tipo: 0
Dívida total (D = A + B + C): 151

Gerado em 2026-09-05 por `dart run tool/debt_report.dart`.

- A.1 chamadas `_dyn(...)` (exclui as 3 linhas de declaração do helper): 102
- A.2 `as dynamic`: 0
- A.3 declarações/parâmetros `dynamic x` (exclui o helper): 47
...
| view_control.dart | 3692 | 34 (26/0/8) | 0 / 0 | 0 | 34 |
