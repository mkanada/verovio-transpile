...
```bash
dart test                                   # full suite (~3 min; 701 tests, all green, measured 2026-09-05)
...
dart run tool/cluster_deltas.dart           # groups every numeric divergence by probable cause →
                                            ...
                                            # Regenerate it after every --all, or the next iteration
                                            # ranks against stale dumps.
dart run tool/probe_diff.dart test/corpus/<fam>/<x>.mei
                                            # pinpointing: the Dart drawing stream (DrawRecorder) vs
                                            # the instrumented C++ fixture, aligned by seq+path →
                                            # fn, seq, path, expected × got (Δ) and "origem provável:
                                            # View::…". --dir=test/corpus --rank groups the FIRST
                                            # divergence of each file by (fn, origem). The rank says
                                            # WHERE a divergence is born; cluster_deltas says what it
                                            # is WORTH. Reads test/fixtures/cpp/05-38 only.
./tool/gen_probe_fixtures.sh <família>      # the fixture probe_diff needs: runs cpp_probe/run.sh
                                            # 05-38 over a family and aborts if the instrumented SVG
...
- **`core/`** — `vrvdef.dart` (the `ClassId` enum, constants, units — the spine everything keys off), `bounding_box.dart` (base class of `Object`), `devicecontextbase.dart` (Pen/Brush/FontInfo/TextExtend), `options_shell.dart` (all 222 active C++ options with options.cpp defaults, measured 2026-09-06; only the commented-out deprecated group and the `m_items` registry itself have no counterpart), `logging.dart`, `fraction.dart`, `smufl.dart`, `tunings.dart`, `file_reader.dart` (conditional `dart:io`/stub import so the package stays web-safe).
...
- `tool/_scratch_*.dart`, `tool/t8.dart`, `tool/dbg_c.dart` are throwaway debug scripts. As of 2026-09-05 (re-measured), `dart analyze` reports **0 issues**: the 11 scratch files that used to carry warnings (`_scratch_debug.dart`, `_scratch_debug2/3/4.dart`, `_scratch_idx.dart`, `_scratch_lig.dart`, `_scratch_metersig.dart`, `_scratch_onsets.dart`, `_scratch_sylprobe2.dart`, `_scratch_sylspan.dart`, `tool/_tmp_loop/pedaldbg.dart`) were unused by anything else in the tree and were deleted rather than cleaned up in place. Don't build on the remaining scratch scripts; if a future one grows warnings, deleting it (once confirmed unused) is preferred over patching it, since these are throwaway by design.
- **⚠️ The rendering type debt was renamed, not paid — and both repo gates are blind to the new spelling.** Re-measured 2026-09-05 (occurrence counts, not line counts). The old numbers (739 `as dynamic`, 820 `catch (_)`, nine `ignore_for_file`, measured 2026-08-29) now read 0, 1 and 0, and that reads like a payoff. It is not: the same code is spelled differently. `as dynamic` became **`_dyn(...)`** — the laundering helper `dynamic _dyn(dynamic o) => o;`, declared in `view_control.dart:46`, `view_element.dart:208` and `view_mensural.dart:74` and called **324 times** (199 / 93 / 32), plus 132 `dynamic x` local declarations. `catch (_)` became **`catch (e) { e.toString(); }`** — 283 of them (152 / 87 / 44) inside **438 catches** in the directory, of which **not one** rethrows or logs. The pair is what matters: `_dyn` makes a member that does not exist compile, and the catch makes the resulting `NoSuchMethodError` vanish at runtime, so a whole drawing branch is skipped in silence and the divergence surfaces far away as a missing or misplaced glyph. **894 raw points of debt**, and both gates report near-zero because `verify_phases.dart --fase=5` (criterion 5.2) and `tool/debt_report.dart` grep for the literal old spellings. Do not add to this pattern under any spelling, and do not "fix" it by inventing a third one. The dedicated loop for paying it down is `prompts/loop-tipagem-prompt-supervisor.md` + `-subagente.md` (diary: `prompts/loop-tipagem-diario.md`); it does not run at the same time as the fidelity loop, since both edit these files and regenerate `test/golden/dart/**`.
...
- `renderSvgForComparison` pins the generated `@xml:id`s (`Object.seedID(kHarnessXmlIdSeed)`, 12345 — the same seed `cpp_probe/run.sh` passes to the C++ `-x`) and renders the first file of a process twice, discarding the cold pass. Both are needed for byte-stable dumps: every `Object` construction bumps the id counter, so a cold process shifts the whole sequence for its first document. Without this, each `--all` rewrote all 621 `test/golden/dart` dumps with pure id noise. It cannot change a comparison verdict — the comparator normalizes ids on both sides.
...
- `test/golden/cpp/**.svg` (621 files, one per corpus file — 623 before the removal) are the C++ reference output and **are** compared against now — by `tool/compare_svg.dart` and by the per-family ratchets in `test/view_*_test.dart`. The fidelity loop (see `prompts/loop-prompt-supervisor.md`) shifts structural/numeric counts every iteration; current totals live in `tool/SVG_VALIDATION.md` (first lines) — do not freeze them in prose here. That report carries **two** scores: lines 3-4 count files fully clean (discrete), lines 5-6 count divergences remaining corpus-wide (continuous). The loop commits on the continuous one — with a median of ~19 distinct deltas per divergent file, the discrete score cannot see a change that removes thousands of divergences without finishing any single file, and the earlier loop kept discarding exactly those. Since 2026-09-05 that ratchet has one documented escape hatch: a line-by-line port that *raises* the score through downstream cascade may still be committed if the C++ `file.cpp:line` is cited, the cascade is named and written into `prompts/loop-diario.md`, and the commit message says `cascata:` — the port's goal is equivalence with the C++, and the score is a proxy for it, not the other way round. `prompts/loop-diario.md` accumulates what each iteration learned, committed even when its code is thrown away. Timemaps (`-t timemap`) and element histograms (`-t mei`) remain the secondary cross-checks.
